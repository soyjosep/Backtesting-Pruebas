...
data['Signal'] = 0
data.iloc[83:, data.columns.get_loc('Signal')] = np.where(data['EMA8'][83:] > data['EMA83'][83:], 1, -1)

# Calcular las posiciones basadas en las señales (1 para comprar, -1 para vender)
data['Position'] = data['Signal']

# Calcular el rendimiento horario
data['Return'] = np.log(data['Close'] / data['Close'].shift(1))

# Calcular el rendimiento de la estrategia (compra y venta)
...
plt.title('Estrategia de Backtesting de Medias Móviles Exponenciales (EMA8 y EMA83) - Compras y Ventas (Bitcoin)')
plt.legend()
plt.show()
