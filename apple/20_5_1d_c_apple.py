...
# Calcular la media móvil exponencial rápida (5 días) y lenta (20 días)
data['EMA5'] = data['Close'].ewm(span=5, adjust=False).mean()
data['EMA20'] = data['Close'].ewm(span=20, adjust=False).mean()

# Crear las señales de compra y cierre de posiciones (sin posiciones en corto)
data['Signal'] = 0
data.iloc[20:, data.columns.get_loc('Signal')] = np.where(data['EMA5'][20:] > data['EMA20'][20:], 1, 0)
...
plt.title('Estrategia de Backtesting de Medias Móviles Exponenciales (EMA5 y EMA20) - Solo Compras - Diario')
plt.legend()
plt.show()
