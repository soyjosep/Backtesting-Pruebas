...
# Calcular la media móvil exponencial rápida (20 días) y lenta (50 días)
data['EMA20'] = data['Close'].ewm(span=20, adjust=False).mean()
data['EMA50'] = data['Close'].ewm(span=50, adjust=False).mean()

# Crear las señales de compra y cierre de posiciones (sin posiciones en corto)
data['Signal'] = 0
data.iloc[50:, data.columns.get_loc('Signal')] = np.where(data['EMA20'][50:] > data['EMA50'][50:], 1, 0)
...
plt.title('Estrategia de Backtesting de Medias Móviles Exponenciales (EMA20 y EMA50) - Solo Compras')
plt.legend()
plt.show()
