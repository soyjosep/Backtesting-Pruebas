...
# Calcular la media móvil exponencial rápida (5 días) y lenta (10 días)
data['EMA5'] = data['Close'].ewm(span=5, adjust=False).mean()
data['EMA10'] = data['Close'].ewm(span=10, adjust=False).mean()

# Crear las señales de compra y cierre de posiciones (sin posiciones en corto)
data['Signal'] = 0
data.iloc[10:, data.columns.get_loc('Signal')] = np.where(data['EMA5'][10:] > data['EMA10'][10:], 1, 0)
...
plt.title('Estrategia de Backtesting de Medias Móviles Exponenciales (EMA5 y EMA10) - Solo Compras - Diario')
plt.legend()
plt.show()
