...
# Descargar datos del activo Apple (AAPL) en intervalos de 5 minutos y período máximo disponible
data = yf.download('AAPL', period='max', interval='5m')

# Calcular la media móvil exponencial rápida (100 períodos) y lenta (1000 períodos)
...
data['Signal'] = 0
data.iloc[1000:, data.columns.get_loc('Signal')] = np.where(data['EMA100'][1000:] > data['EMA1000'][1000:], 1, 0)
...
plt.title('Estrategia de Backtesting de Medias Móviles Exponenciales (EMA100 y EMA1000) - Solo Compras - 5min')
plt.legend()
plt.show()
...
sharpe_ratio = data['Strategy'].mean() / data['Strategy'].std() * np.sqrt(252 * 78)  # 252 días, 78 intervalos de 5m por día
