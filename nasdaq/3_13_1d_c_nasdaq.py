...
def backtesting_medias_moviles_exponenciales(ticker, fecha_inicio, fecha_fin):
    try:
        ...
        # Calcular medias móviles exponenciales de 3 y 13 periodos
        data['EMA3'] = data['Close'].ewm(span=3, adjust=False).mean()
        data['EMA13'] = data['Close'].ewm(span=13, adjust=False).mean()

        # Generar señales de compra (1) y mantener posición (0)
        data['Signal'] = 0
        data.loc[data['EMA3'] > data['EMA13'], 'Signal'] = 1

        # Calcular retornos diarios
        data['Retornos'] = data['Close'].pct_change()

        # Calcular retornos de la estrategia solo en posiciones largas
...
print(f"Beneficio total de la estrategia combinada utilizando EMAs de 3 y 13 periodos: {beneficio_total:.2f}%")
