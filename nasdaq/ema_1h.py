...
def fetch_data(ticker, start_date, end_date, retries=3, delay=5):
    ...
    while attempt < retries:
        try:
            ...
            min_required_data = 100  # Al menos 100 puntos de datos
            if len(data) < min_required_data:
                print(f"Ticker: {ticker} | Datos insuficientes ({len(data)} puntos).")
                return None

            # Imprimir las columnas descargadas y cantidad de datos para depuración
...
def backtest_individual(all_data, start_ma, end_ma):
    best_results = {}
    ...
    for ticker, data in all_data:
        ...
        print(f"\nIniciando backtest individual para {ticker} con {len(close_series)} puntos de datos.")
        ...
        for fast_ma in range(start_ma, max_ma + 1):
            for slow_ma in range(fast_ma + 1, max_ma + 1):
                try:
                    ...
                    fast_ma_series = close_series.ewm(span=fast_ma, adjust=False).mean()
                    slow_ma_series = close_series.ewm(span=slow_ma, adjust=False).mean()
...
def backtest_global(all_data, start_ma, end_ma):
    best_results = {}
    global_best_profit = float('-inf')
    global_best_fast_ma = None
    global_best_slow_ma = None

    total_tickers = len(all_data)
    print(f"\nIniciando backtest global para {total_tickers} tickers.")

    # Definir el rango de medias móviles
    for fast_ma in range(start_ma, end_ma + 1):
        for slow_ma in range(fast_ma + 1, end_ma + 1):
            total_profit = 0
            print(f"\nEvaluando combinación Fast MA: {fast_ma}, Slow MA: {slow_ma}")
            ...
            for ticker, data in all_data:
                ...
                try:
                    ...
                    # Calcular las ganancias
                    profits_list = sell_prices.values - buy_prices.values
                    ticker_profit = profits_list.sum()
            ...
            print(f"Combinación Fast MA: {fast_ma}, Slow MA: {slow_ma} | Profit Total: {total_profit:.2f}")

            # Verificar y actualizar el mejor rendimiento global
            if total_profit > global_best_profit:
                global_best_profit = total_profit
                global_best_fast_ma = fast_ma
                global_best_slow_ma = slow_ma

    if global_best_fast_ma and global_best_slow_ma:
        best_results = {
            'Global_Best_Fast_MA': global_best_fast_ma,
            'Global_Best_Slow_MA': global_best_slow_ma,
            'Total_Profit': global_best_profit
        }
        print(f"\nMejor combinación global encontrada | Fast MA: {global_best_fast_ma} | Slow MA: {global_best_slow_ma} | Profit Total: {global_best_profit:.2f}")
    else:
        print("No se encontró una combinación global válida.")
...
def main():
    ...
    # Realizar el backtesting individual para cada ticker
    individual_results = backtest_individual(all_data, 1, 200)  # Rango reducido a 1-200

    # Realizar el backtesting global para todas las empresas
    global_results = backtest_global(all_data, 1, 200)  # Rango reducido a 1-200

    # Resumen de los mejores resultados individuales
    if individual_results:
        print("\nResumen de las mejores combinaciones de medias móviles por ticker:")
        summary_df = pd.DataFrame(individual_results).T
        summary_df_sorted = summary_df.sort_values(by='Profit', ascending=False)
        print(summary_df_sorted)

        # Guardar los resultados individuales en un archivo CSV
        summary_df_sorted.to_csv('best_moving_averages_individual.csv', index=True)
        print("\nLos resultados individuales se han guardado en 'best_moving_averages_individual.csv'.")
    else:
        print("No se encontraron resultados válidos para ningún ticker en el backtest individual.")

    # Resumen de los mejores resultados globales
    if global_results:
        print("\nResumen del mejor cruce de medias móviles global:")
        summary_global_df = pd.DataFrame([global_results])
        print(summary_global_df)

        # Guardar los resultados globales en un archivo CSV
        summary_global_df.to_csv('best_moving_averages_global.csv', index=False)
        print("\nLos resultados globales se han guardado en 'best_moving_averages_global.csv'.")
    else:
        print("No se encontraron resultados válidos en el backtest global.")
