...
def obtener_rango_comun_fechas(tickers):
    ...
    for ticker in tickers:
        try:
            ...
            data = yf.download(ticker, interval='1h', period='max', progress=False)
            if data.empty:
    ...
    print(f"\nRango común de fechas: {fecha_inicio_comun} - {fecha_fin_comun}")
...
def backtesting_medias_moviles_exponenciales(ticker, fecha_inicio, fecha_fin):
    try:
        ...
        data = yf.download(tickers=ticker, interval='1h', start=fecha_inicio, end=fecha_fin, progress=False)
        print(f"Datos descargados para {ticker}: {len(data)} filas.")
        if data.empty:
            print(f"No se pudo obtener datos para {ticker}.")
            return

        # Calcular medias móviles exponenciales de 18 y 78 periodos
        data['EMA18'] = data['Close'].ewm(span=18, adjust=False).mean()
        data['EMA78'] = data['Close'].ewm(span=78, adjust=False).mean()

        # Generar señales de compra (1) y venta (-1)
        data['Signal'] = 0
        data.loc[data['EMA18'] > data['EMA78'], 'Signal'] = 1
        data.loc[data['EMA18'] < data['EMA78'], 'Signal'] = -1
...
# Calcular el retorno promedio horario de la estrategia (suponiendo igual peso)
...
# Calcular el beneficio/pérdida horario promedio en porcentaje
beneficio_horario_promedio = df_estrategia['Retorno_Promedio'].mean() * 100

# Mostrar los resultados
print("\nResultados del Backtesting:")
print(f"Beneficio total de la estrategia combinada utilizando EMAs de 18 y 78 periodos: {beneficio_total:.2f}%")
print(f"Beneficio/Pérdida horario promedio: {beneficio_horario_promedio:.5f}%")
print(f"Período de análisis: {fecha_inicio_comun} - {fecha_fin_comun}")