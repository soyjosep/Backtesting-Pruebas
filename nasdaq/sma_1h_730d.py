...
def moving_average_strategy(data, fast_period, slow_period):
    """Función que implementa la estrategia de cruce de medias móviles."""
    ...
    signal = np.where(data[f'SMA_{fast_period}'] > data[f'SMA_{slow_period}'], 1, 0)
    ...
    # Aseguramos que profit es un valor numérico y no una Serie
    return profit if not isinstance(profit, pd.Series) else float(profit.iloc[0])
...
def backtest_company(ticker):
    ...
    for fast, slow in product(range(1, 51), repeat=2):
        if fast < slow:
